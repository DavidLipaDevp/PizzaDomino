...
public interface OrderRepository extends ListCrudRepository<OrderEntity,Integer> {
    ...
    //personalizacion de query con projection
    @Query(value = " SELECT po.id_order AS idOrder, cu.name AS customerName, " +
            "       po.date AS orderDate, po.total AS orderTotal, STRING_AGG(pi.name, ', ') AS pizzaNames " +
            " FROM pizza_order po " +
            " INNER JOIN customer cu ON po.id_customer = cu.id_customer " +
            " INNER JOIN orden_item oi ON po.id_order = oi.id_order " +
